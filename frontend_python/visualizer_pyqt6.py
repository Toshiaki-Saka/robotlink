"""
3-DOF Robot Arm Simulation Visualizer — PyQt6 edition
Embeds matplotlib figures inside a QMainWindow; output is identical to good.png.

Usage:
    python visualizer_pyqt6.py [path/to/sim_results.csv]
...
class MainWindow(QMainWindow):
    ...
    def _build_central(self):
        ...
        # Right: 3-D animation (always visible)
        right = QWidget()
        ...
        # Force an even 50/50 split so the 3-D animation gets half the width.
        # (Stretch factors only distribute *extra* space; the left tab area's
        # larger size hint would otherwise leave the animation cramped.)
        ...
        def run():
            try:
                ...
                if result.returncode == 0:
                    ...
                    QTimer.singleShot(0, lambda: self._show_sim_error(err))
    ...
    def _setup_animation(self):
        ...
        lay = self._anim_container.layout()
        if lay is None:
